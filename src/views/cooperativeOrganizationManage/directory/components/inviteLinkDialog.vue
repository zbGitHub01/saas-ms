  ...
  <el-dialog v-model="dialogVisible" title="邀请链接" width="450px" :before-close="handleClose">
    <div style="margin: 0 54px">
      ...
      >
        ...
        <el-form-item v-if="optionData.orgList.length > 0" label="请选择被邀请机构：" prop="orgId">
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <template v-if="link.type === 2">
          <el-button v-copy="link.url" type="primary">复制邀请链接</el-button>
        </template>
        <template v-if="link.type === 3">
          <el-button v-copy="link.url" type="primary">复制准入提交页面地址</el-button>
        </template>
      </span>
    </template>
  </el-dialog>
